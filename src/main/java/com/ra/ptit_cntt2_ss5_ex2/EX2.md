...
| **3. Thêm sách mới** | **POST** | `/books` | *Không có*  | `201 Created` | `400 Bad Request` *(Thiếu trường bắt buộc hoặc dữ liệu sai định dạng)* |
| **4. Cập nhật số lượng sách** *(Chỉ cập nhật 1 trường)* | **PATCH** | `/books/:id` |  *Không có*  | `200 OK` | `404 Not Found` *(ID sách không tồn tại)* |
...
| **7. Tạo thẻ mượn sách mới** | **POST** | `/loans` | *Không có*  | `201 Created` | `400 Bad Request` *(Sách đã hết số lượng để cho mượn)* |
| **8. Trả sách** *(Cập nhật ngày trả thực tế)* | **PATCH** | `/loans/:id` | *Không có*  | `200 OK` | `404 Not Found` *(Mã số thẻ mượn không tồn tại)* |
